...
`, k = (p, a) => {
  const s = new Date(a), e = String(s.getMilliseconds()).padStart(3, "0");
  return `Friends join at http://<this-computer’s-Wi-Fi-IP>:${p} · live ${s.toLocaleTimeString()}.${e}`;
}, I = y.slice(y.indexOf("<main"), y.indexOf("</main>") + 7), O = E({
  mount(p, a) {
    const s = document.createElement("style");
    s.textContent = N + R + A + j, document.head.append(s), p.innerHTML = I, document.body.classList.add("poker-preview");
    ...
    const b = e.querySelector("#lan-error"), r = document.createElement("p");
    r.id = "lan-share", r.setAttribute("role", "status"), r.style.color = "#c1db9c";
    const l = (t) => {
      r.isConnected || e.querySelector(".side-panel").append(r), r.textContent = t;
    }, d = a.api.services, c = a.api.net;
    let n = !1, v = !1;
    const g = async (t, o) => {
      if (v) {
        l(k(t, o));
      ...
      try {
        l(k(t, o));
        const { setApiTransport: i } = await import("./client-cDCznDce.js");
        i(w()), e.hidden = !0, n = !1;
      } catch (i) {
    ...
    return e.querySelector("#lan-host").addEventListener("click", () => {
      ...
      n || (n = !0, b.textContent = "Starting LAN host…", (async () => {
        try {
          ...
          const { setApiTransport: o } = await import("./client-cDCznDce.js");
      ...
      S(async () => {
        const { setApiTransport: h } = await import("./client-cDCznDce.js");
        h(C((T) => c.fetch(T), i));
      });
    }), () => {
      L(), e.remove(), s.remove();
