...
import { S as y, p as w, n as S } from "./inAppTransport-CvkJA9Og.js";
...
`, j = x.slice(x.indexOf("<main"), x.indexOf("</main>") + 7), F = E({
  mount(b, n) {
    const l = document.createElement("style");
    l.textContent = C + R + A + P, document.head.append(l), b.innerHTML = j, document.body.classList.add("poker-preview");
    ...
    t.className = "panel-scrim", t.style.position = "absolute", t.style.zIndex = "40", t.innerHTML = `
      ...
      </aside>`, b.querySelector("#entry")?.append(t);
    const u = t.querySelector("#lan-error"), a = document.createElement("p");
    ...
    }, d = n.api.services, p = n.api.net;
    let s = !1, v = !1;
    const g = async (e, o) => {
      if (v) {
        r(`Friends join at http://<this-computer’s-Wi-Fi-IP>:${e} · updated ${new Date(o).toLocaleTimeString()}`);
        return;
      }
      v = !0, s = !0;
      try {
        ...
        s = !1, v = !1, c(i instanceof Error ? i.message : String(i));
      }
    }, m = n.runtime.state();
    m?.running && m.port && g(m.port, m.at ?? Date.now());
    const k = n.runtime.subscribe((e) => {
      const o = e;
    ...
    async function L(e, o) {
      if (!s) {
        ...
        try {
          ...
          s = !1, c(i instanceof Error ? i.message : String(i));
        }
      }
    }
    return t.querySelector("#lan-host").addEventListener("click", () => {
      if (!d) {
        c("This Agent Code build does not support extension services.");
        return;
      }
      s || (s = !0, u.textContent = "Starting LAN host…", (async () => {
        try {
          await d.start(y);
          const e = await d.expose(y, !0);
          if (!e.lan || !e.port) throw new Error("LAN exposure was not granted.");
          const { setApiTransport: o } = await import("./client-BXRLvBPC.js");
          o(w()), t.hidden = !0, r(`Friends join at http://<this-computer’s-Wi-Fi-IP>:${e.port} — then use Create table below.`);
        } catch (e) {
          s = !1, c(e instanceof Error ? e.message : String(e));
        }
      })());
    }), t.querySelector("#lan-join").addEventListener("submit", (e) => {
      if (e.preventDefault(), !p) {
        c("This Agent Code build does not support brokered fetch.");
        return;
      }
      ...
      try {
        ...
        c(h instanceof Error ? h.message : "Invalid host address.");
        return;
      }
      L(async () => {
