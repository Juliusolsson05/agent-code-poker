import { d as N } from "./runtime-XLX8az2X.js";
import { s as P } from "./styles-DJYLcAU6.js";
import { S, p as I, n as V } from "./inAppTransport-CvkJA9Og.js";
...
function B(e) {
  ...
  if (!o) throw n();
  const [, r, b] = o;
  if (b && Number(b) > 65535) throw n();
  if (r !== "localhost") {
    const p = r.split(".");
    if (p.some((c) => String(Number(c)) !== c || Number(c) > 255)) throw n();
    const [s, u] = p.map(Number);
    if (!(s === 127 || s === 10 || s === 192 && u === 168 || s === 172 && u >= 16 && u <= 31)) throw n();
  }
  return new URL(t).origin + "/";
}
const te = "audio/mpeg", oe = 96 * 1024;
function D(e) {
  if (e.length < 4) return !1;
  if (e[0] === 73 && e[1] === 68 && e[2] === 51) return e[3] >= 2 && e[3] <= 4;
  if (e[0] !== 255 || (e[1] & 224) !== 224) return !1;
  const t = e[1] >> 3 & 3, o = e[1] >> 1 & 3, n = e[2] >> 4, r = e[2] >> 2 & 3;
  return t !== 1 && o !== 0 && n !== 15 && r !== 3;
}
function ne(e) {
  let t = "";
...
const q = "https://api.elevenlabs.io", H = "mp3_22050_32", U = "eleven_flash_v2_5", ae = {
    ...
    async synthesize(o) {
      ...
      try {
        r = await t({
          ...
          url: `${q}/v1/text-to-speech/${n.voiceId}?output_format=${H}`,
          headers: { "xi-api-key": n.apiKey, "content-type": "application/json", accept: "audio/mpeg" },
          body: JSON.stringify({ text: o, model_id: U })
        });
      } catch {
        return { ok: !1, reason: "network" };
      }
      return r.status < 200 || r.status >= 300 ? { ok: !1, reason: Y(r.status, r.bytes) } : r.bytes.length > $ || !D(r.bytes) ? { ok: !1, reason: "invalid-response" } : { ok: !0, audio: r.bytes };
    }
  };
}
...
const C = 15e3;
function J(e, t = C) {
  return async ({ url: o, headers: n, body: r }) => {
    let b;
    const p = new Promise((c, l) => {
      b = setTimeout(() => l(new Error("Voice request timed out.")), t);
    }), s = await Promise.race([e(o, {
      httpMethod: "POST",
      body: r,
      responseType: "base64",
      headers: Object.entries(n).map(([c, l]) => ({ name: c, value: l }))
    }), p]).finally(() => clearTimeout(b));
    if (s.bodyEncoding !== "base64") throw new Error("This Agent Code build cannot return binary responses.");
    const u = j(s.body);
    if (!u) throw new Error("The host returned an invalid body.");
    return { status: s.status, contentType: s.contentType, bytes: u };
  };
}
const W = `
...
}, X = v.slice(v.indexOf("<main"), v.indexOf("</main>") + 7), le = N({
  mount(e, t) {
    const o = document.createElement("style");
    o.textContent = z + P + M + W, document.head.append(o), e.innerHTML = X, document.body.classList.add("poker-preview");
    ...
    const b = n.querySelector("#lan-error"), p = document.createElement("p");
    p.id = "lan-share", p.setAttribute("role", "status"), p.style.color = "#c1db9c";
    const s = (a) => {
      p.isConnected || n.querySelector(".side-panel").append(p), p.textContent = a;
    }, u = t.api.services, c = t.api.net;
    let l = !1, y = !1;
    const g = async (a) => {
      const i = await import("./client-BPsZBfi6.js");
      i.setVoiceEnvironment({
        store: G(t.api),
        http: J(c ? (d, f) => c.fetch(d, f) : async () => {
      ...
      y = !0, l = !0;
      try {
        s(L(a, i)), await g(I()), n.hidden = !0, l = !1;
      } catch (d) {
        l = !1, y = !1, h(d instanceof Error ? d.message : String(d));
      }
    }, m = t.runtime.state();
    m?.running && m.port && T(m.port, m.at ?? Date.now());
    const _ = t.runtime.subscribe((a) => {
      const i = a;
      i?.running && i.port && T(i.port, i.at ?? Date.now());
    }), h = (a) => {
      b.textContent = a;
    };
    async function R(a, i) {
      if (!l) {
        ...
        try {
          ...
          l = !1, h(d instanceof Error ? d.message : String(d));
        }
      }
    }
    return n.querySelector("#lan-host").addEventListener("click", () => {
      ...
      l || (l = !0, b.textContent = "Starting LAN host…", (async () => {
        try {
          await u.start(S);
          const a = await u.expose(S, !0);
          if (!a.lan || !a.port) throw new Error("LAN exposure was not granted.");
          await g(I()), n.hidden = !0, s(`Friends join at http://<this-computer’s-Wi-Fi-IP>:${a.port} — then use Create table below.`);
        } catch (a) {
          l = !1, h(a instanceof Error ? a.message : String(a));
        }
      })());
    }), n.querySelector("#lan-join").addEventListener("submit", (a) => {
