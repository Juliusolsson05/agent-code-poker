...
export function buildPictureRelief(pictures: SurroundPicture[]): THREE.Group {
  ...
  const relief = new VoxelSculpt(.025)
  ...
  for (const p of pictures) {
    ...
    // A 25mm backing stays behind every relief layer and in front of the
    // wallpaper. Instanced backings share one geometry and material.
    const [x, y, z] = p.position
    dummy.position.set(x, y, z); dummy.rotation.y = p.facing === '+x' ? Math.PI / 2 : p.facing === '-x' ? -Math.PI / 2 : Math.PI
    dummy.scale.set(p.size[0], p.size[1], .009); dummy.updateMatrix()
    ...
    const layer = (d: number, color: string, inside: (u: number, v: number) => boolean, u0 = -1, u1 = 1, v0 = -1, v1 = 1) => {
      const a = world(u0, v0, d - .0125), b = world(u1, v1, d + .0125)
      ...
      relief.volume(min, max, (px, py, pz) => { const q = local(px, py, pz); return Math.abs(q.d - d) < .014 && inside(q.u, q.v) }, color)
    }
    if (winter || p.kind === 'landscape') {
      // Low-poly silhouettes have parallax: far blue hills sit behind a second
      // treeline; near snow is light enough to read even in the warm room.
      layer(.011, winter ? '#52647a' : '#788875', (u, v) => v < -.22 + .12 * Math.sin(u * 4.2) + .045 * Math.sin(u * 11))
      layer(.018, winter ? '#284038' : '#354b35', (u, v) => v < -.39 + .07 * Math.sin(u * 6.7))
      for (let i = 0; i < 13; i++) {
        const u = -.93 + i * .155, h = .18 + ((i * 7) % 5) * .034, base = -.38 + .05 * Math.sin(u * 6)
        layer(.023, i % 3 ? '#18342e' : '#244238', (a, v) => v >= base && v <= base + h && Math.abs(a - u) < .013 + (base + h - v) * .24,
          u - .11, u + .11, base, base + h)
      }
      layer(.029, winter ? '#c5d3dc' : '#aab39a', (u, v) => v < -.70 + .07 * Math.sin(u * 5.1) + .028 * Math.cos(u * 13))
      if (winter) {
        layer(.023, '#e3e9e6', (u, v) => (u - .55) ** 2 + (v - .55) ** 2 < .035, .30, .80, .30, .80)
        ...
        layer(.031, '#4b3328', (u, v) => u > -.55 && u < -.2 && v > -.67 && v < -.46, -.6, -.15, -.7, -.4)
        layer(.037, '#e3e7e5', (u, v) => u > -.60 && u < -.15 && v > -.48 && v < -.45 - Math.abs(u + .375) * 1.0, -.62, -.13, -.5, -.27)
        for (const u of [-.47, -.28]) layer(.040, '#ffc176', (a, v) => Math.abs(a - u) < .035 && v > -.61 && v < -.52, u - .04, u + .04, -.62, -.51)
      }
      ...
      layer(.017, '#755639', (u, v) => (u / .56) ** 2 + ((v + .15) / .73) ** 2 < 1)
      layer(.025, '#9b7450', (u, v) => (u / .27) ** 2 + ((v - .26) / .31) ** 2 < 1)
      for (const s of [-1, 1]) layer(.03, '#483124', (u, v) => ((u - s * .29) / .16) ** 2 + ((v - .19) / .32) ** 2 < 1)
      layer(.036, '#8c3030', (u, v) => Math.abs(u) < .25 && v > -.04 && v < .035)
    } else if (p.kind === 'clock') {
      layer(.012, '#eee1bd', (u, v) => u * u + v * v < .85)
      for (let i = 0; i < 12; i++) { const a = i / 12 * Math.PI * 2, tx = Math.sin(a) * .68, ty = Math.cos(a) * .68
        ...
        layer(.023, '#2a2018', (u, v) => Math.abs(u - tx) < .095 && Math.abs(v - ty) < .095, tx - .10, tx + .10, ty - .10, ty + .10)
      }
      layer(.029, '#2a2018', (u, v) => Math.abs(v - u * .55) < .095 && u > -.05 && u < .50, -.1, .55, -.1, .32)
      layer(.034, '#2a2018', (u, v) => Math.abs(u + v * .15) < .07 && v > 0 && v < .62, -.13, .05, 0, .65)
    } else {
      for (const base of [-.45, .25]) for (let i = 0; i < 5; i++) {
        const v = base + i * .075
        layer(.014, '#5b4a37', (_u, yv) => Math.abs(yv - v) < .008, -.90, .90, v - .015, v + .015)
      }
      for (let i = 0; i < 13; i++) { const u = -.78 + i * .12, v = (i % 2 ? -.31 : .39) + (i % 5) * .075
        layer(.026, '#382b22', (a, yv) => ((a - u) / .035) ** 2 + ((yv - v) / .035) ** 2 < 1, u - .05, u + .05, v - .05, v + .05)
      }
    }
  }
