...
import { cardCount, type SceneState } from '../presentation/RoomProjection'
...
export class CardField {
  ...
  update(state: SceneState, now = performance.now() / 1000): void {
    const old = this.previous, newHand = old?.dealId !== state.dealId
    if (newHand) {
    ...
    const show = state.publicShowdown, wasShow = old?.publicShowdown
    for (const p of state.players) {
      if (cardCount(p.cards) !== 2) continue
      ...
      for (let i = 0; i < 2; i++) {
        ...
        if (show && (newHand || !wasShow) && !p.folded && p.cards.kind === 'visible') {
          const mesh = this.card(key, p.cards.values[i])
  ...
  private updateInspection(): void {
    ...
    const values = player?.cards.kind === 'visible' ? player.cards.values : []
    const visible = this.inspecting && !!player && !player.folded && values.length === 2 && !state?.publicShowdown
    ...
    this.inspectedCards.forEach((mesh, i) => {
      ...
      if (visible) {
        ...
        material.map = this.texture(values[i]); material.needsUpdate = true
      }
    })
  }
